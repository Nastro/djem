...
Ext.define('djem.view.main.GridController', {
  ...
  changeFilter: function() {
    var me = this, store = me.getView().getStore(), value = me.getFilter().getValue();
  ...
  init: function() {
    var me = this, view = me.getView(), main = view.up('app-main'), toolbar = main.getToolbar();
  ...
  initViewModel: function() {
    ...
    store.on('metachange', function(_store, meta) {
      ...
      if (meta.view) {
        ...
        gridView.show();
        gridView.add(Ext.create('widget.main-content', { data: meta }));
        return;
      }
      gridView.hide();
      ...
      view.on({ destroy: function() { me.destroyFilterListeners(); }, options: { single: true } });
    ...
    var contextMenu = new Ext.menu.Menu({
      listeners: {
        hide: function(_this) {
          ...
          Ext.defer(function() { _this.destroy(); }, 1);
        }
      },
      items: menu
    });
